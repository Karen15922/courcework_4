...
def get_vacancies_by_salary(vacancies, salary_range):
    ...
    for vacancy in vacancies:
        if vacancy.salary > salary_range[0] and vacancy.salary < salary_range[1]:
...
def user_interaction():
    ...
    if user_select == 1:
        ...
        filter_words = input("Введите ключевые слова для фильтрации вакансий: \033[32mНазвания городов через пробел\033[0m\n").split()
        try:
            salary_range = list(map(int, input("Введите диапазон зарплат: например \033[32m100000-150000\033[0m\n").split('-')))
        except ValueError:
